...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'data/ques1.txt':
            with open('data/ques2.txt','r') as f3:
                for line in f3:
                    if line != '\n':
                        label_num, question = line.split("\t")
                        label, number = label_num.split("||#")
        ...
        if filename == 'data/ques1.txt':
            joblib.dump(train,'model2/train18.pkl')
        else:
            joblib.dump(train,'model2/test18.pkl')
    return train

def training():
    train = load_model('model2/train18.pkl')
    if train is None:
        train = load_data('data/ques1.txt')
    print "---------------------------"
    print "Training"
    print "---------------------------"
    vectorizer = load_model('model2/vectorizer18.pkl')
    ...
    y_train = train["label"]
    joblib.dump(vectorizer, 'model2/vectorizer18.pkl')
    s = set(y_train)
    print s
    print len(s)
...
def fit1(X_train,y_train):
    ...
    uni_big.fit(X_train, y_train)
    joblib.dump(uni_big, 'model2/uni_big18.pkl')

def test_file():
    uni_big = load_model('model2/uni_big18.pkl')
    if uni_big is None:
        training()
    uni_big = load_model('model2/uni_big18.pkl')
    vectorizer = load_model('model2/vectorizer18.pkl')
    test = load_model('model2/test7.pkl')
    if test is None:
    ...
    with open('result/fail18.txt',"w") as f:
