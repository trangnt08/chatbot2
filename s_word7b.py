...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'data/ques7a.txt':
            with open('data/ques7b.txt','r') as f3:
                for line in f3:
                    if line != '\n':
                        label_num, question = line.split("\t")
                        label, number = label_num.split("||#")
        ...
        if filename == 'data/ques7a.txt':
            joblib.dump(train,'model2/train7b.pkl')
        else:
            joblib.dump(train,'model2/test7b.pkl')
    return train

def training():
    train = load_model('model2/train7b.pkl')
    if train is None:
        train = load_data('data/ques7a.txt')
    print "---------------------------"
    print "Training"
    print "---------------------------"
    vectorizer = load_model('model2/vectorizer7b.pkl')
    ...
    y_train = train["label"]
    joblib.dump(vectorizer, 'model2/vectorizer7b.pkl')
    s = set(y_train)
    print s
    print len(s)
...
def fit1(X_train,y_train):
    ...
    uni_big.fit(X_train, y_train)
    joblib.dump(uni_big, 'model2/uni_big7b.pkl')

def test_file():
    uni_big = load_model('model2/uni_big7b.pkl')
    if uni_big is None:
        training()
    uni_big = load_model('model2/uni_big7b.pkl')
    vectorizer = load_model('model2/vectorizer7b.pkl')
    test = load_model('model2/test7b.pkl')
    if test is None:
        test = load_data('data/test7.txt')
