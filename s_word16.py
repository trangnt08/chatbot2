...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'data/ques3.txt':
            joblib.dump(train,'model2/train16c.pkl')
        else:
            joblib.dump(train,'model2/test16c.pkl')
    return train

def training():
    train = load_model('model2/train16c.pkl')
    if train is None:
        train = load_data('data/ques3.txt')
    print "---------------------------"
    print "Training"
    print "---------------------------"
    vectorizer = load_model('model2/vectorizer16c.pkl')
    ...
    y_train = train["label"]
    joblib.dump(vectorizer, 'model2/vectorizer16c.pkl')
    s = set(y_train)
    print s
    print len(s)
...
def fit1(X_train,y_train):
    ...
    uni_big.fit(X_train, y_train)
    joblib.dump(uni_big, 'model2/uni_big16c.pkl')

def test_file():
    uni_big = load_model('model2/uni_big16c.pkl')
    if uni_big is None:
        training()
    uni_big = load_model('model2/uni_big16c.pkl')
    vectorizer = load_model('model2/vectorizer16c.pkl')
    test = load_model('model2/test16c.pkl')
    if test is None:
    ...
    print "confuse matrix: \n", confusion_matrix(y_test, y_pred,
                                                 labels=["0", "2", "3", "4","6", "8", "10",
                                                         "15", "16", "18", "20", "21", "22",
                                                         "24", "26", "27", "30", "31"])
    with open('result/fail16c.txt',"w") as f:
