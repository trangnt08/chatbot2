...
def load_data(filename):
    ...
    with open(filename, 'r') as f:
        ...
        if filename == 'data/ques1.txt':
            joblib.dump(train,'model2/train7.pkl')
        else:
            joblib.dump(train,'model2/test7.pkl')
    return train

def training():
    train = load_model('model2/train7.pkl')
    if train is None:
    ...
    y_train = train["label"]
    joblib.dump(vectorizer, 'model2/vectorizer7.pkl')
    s = set(y_train)
    print s
    print len(s)
...
def test_file():
    ...
    with open('result/fail7.txt',"w") as f:
